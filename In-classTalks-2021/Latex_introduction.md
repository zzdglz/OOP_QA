...
\[ \lim_{x\to0}x^2   \quad\quad\quad   \int_a^b x^2 dx \]
\[ \lim\nolimits _{x \to 0} x^2   \quad\quad\quad   \int\nolimits_a^b x^2 dx \]
```
...
在这里，Markdown不能很好地原生编译它们。只需知道带`*`的是不带每行公式编号的，其余的每行都有公式编号。
...
其他一些好玩的东西，Markdown对Latex支持有限，贴上Tex源码。
